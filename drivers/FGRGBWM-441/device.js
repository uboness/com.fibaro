...
class FibaroRGBWControllerDevice extends ZwaveDevice {
        ...
        */
		this.registerMultipleCapabilityListener(['light_saturation', 'light_hue', 'dim'], async (newValue, opts) => {
			...
			if (this.getCapabilityValue('light_mode') === 'temperature' && typeof newValue.light_hue !== 'number' && typeof newValue.light_saturation !== 'number') {
				const originalHSV = this._tempToHSV(this.getCapabilityValue('light_temperature'));
				const rgb = utils.convertHSVToRGB({ hue: originalHSV.h / 360, saturation: originalHSV.s, value: newValue.dim });
				const rgbw = this._convertRGBtoRGBW({ red: rgb.red, green: rgb.green, blue: rgb.blue });
		...
		this.registerCapabilityListener('light_temperature', async (value, opts) => {
			let red,
				...
				blue,
				white;
			const rgb = this._tempToRGB(value);
			const rgbw = this._convertRGBtoRGBW({ red: rgb.red, green: rgb.green, blue: rgb.blue });
		...
		this.registerCapabilityListener('light_mode', async (value, opts) => {
			if (value === 'color') {
				...
				let red,
					...
					blue,
					white;
				const rgb = this._tempToRGB(this.getCapabilityValue('light_temperature'));
				const rgbw = this._convertRGBtoRGBW({ red: rgb.red, green: rgb.green, blue: rgb.blue });
     ...
     */
	_tempToRGB(value) {
		const hsv = this.temperatureGradient.hsvAt(value)._originalInput;
		...
		if (args.hasOwnProperty('range')) {
			...
			try {
				if (args.range === 'rgb') {
					...
					if (hue) {
						rgb = utils.convertHSVToRGB({ hue, saturation: 1, value: this.getCapabilityValue('dim') });
	...
	_convertRGBtoRGBW({ red, green, blue }) {
		...
		const colorMax = Math.max(red, Math.max(green, blue));

		// If the max is 0 immediately return true black
		if (colorMax === 0) return { red: 0, green: 0, blue: 0, white: 0 };

		// Calculate colour hues
		const multiplier = 255 / colorMax;
		const redHue = red * multiplier;
		const greenHue = green * multiplier;
		const blueHue = blue * multiplier;

		// Calculate the luminance
		const trueMax = Math.max(redHue, Math.max(greenHue, blueHue));
		const trueMin = Math.min(redHue, Math.min(greenHue, blueHue));
		const luminance = ((trueMax + trueMin) / 2) - 127.5 * (2 / multiplier);
	...
	_reportParser(report, channel) {
		...
		const newColour = utils.convertRGBToHSV({ red: this.currentRGB.r, green: this.currentRGB.g, blue: this.currentRGB.b });
